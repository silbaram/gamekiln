...
Completion: begin with a short Korean read-back (1-3 lines) of the understood hypothesis and key build choices applied (controls, win/lose condition, starting values) for user verification before playtesting; if it is wrong, fix the prototype before any play session. Then provide a prototype that can be opened or run to test the hypothesis without extra explanation, record this build as a line in `iterations.md` (with the prior version archived as `prototype-v<K>` if changed), include run instructions, remind that the code is disposable, and remind the user to record playtest evidence in `prototypes/playtest.md` using Facts and Interpretations.

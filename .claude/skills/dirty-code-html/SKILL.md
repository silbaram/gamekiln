...
- Version numbers come from `iterations.md` (single source of truth): the current `prototype.html` is `v<K>` where K = the highest line-starting `v<N>:` entry recorded in `iterations.md`. First build: write `prototype.html` and create `iterations.md` with a `v1:` baseline line (no header line; one v-line per build).
- Before changing an existing `prototype.html` that already has a recorded line-starting `v<N>:` entry, copy it to `prototype-v<K>.html` (K = its recorded version, the highest line-starting `v<N>:` entry in `iterations.md`), then write the new build as `prototype.html`. Never edit a recorded version in place; never overwrite an existing `prototype-v*`.

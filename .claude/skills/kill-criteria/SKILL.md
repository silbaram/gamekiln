...
- Stage 4: recommend retry when the current decision record or batch evidence can be repaired without invalidating its VS basis. Recommend regression to Stage 3 when production evidence breaks the VS basis or a supposedly verified system decision. Recommend kill review when cumulative cost, quality, or feasibility evidence leaves no defensible approved scope.
